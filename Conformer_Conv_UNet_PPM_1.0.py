...
def merge_PPM(entered_input, entered_input2, entered_input3, entered_input4, filters=48):
    merged = layers.concatenate([entered_input, entered_input2, entered_input3, entered_input4], axis=-1)
    conv1 = layers.Conv2D(filters, kernel_size=1, padding='same')(merged)
    enc1 = layers.BatchNormalization()(conv1)
    relu = layers.ReLU()(enc1)
...
def CONFORMER_CONV_UNET_PPM(image_size, num_classes,activation):
    ...
    print("Shape of skip_list[2]:", skip_list[3].shape)
    print("Shape of up2:", up1.shape)
...
def UNet_Enconder(input):
    ...
    skip1, encoder_1 = conformerConvModule(input1, n_filtro,1)
    skip1, teste_1 = conformerConvModule(input1, n_filtro,3)
    skip1, teste_2 = conformerConvModule(input1, n_filtro,7)
    skip1, teste_3 = conformerConvModule(input1, n_filtro,11)

    skip2, encoder_2 = merge_PPM(encoder_1,teste_1,teste_2,teste_3,n_filtro*2)
 
    skip3, encoder_3 = conformerConvModule(encoder_2, n_filtro*4,1)
    skip3, teste_7 = conformerConvModule(encoder_2, n_filtro*4,3)
    skip3, teste_8 = conformerConvModule(encoder_2, n_filtro*4,7)
    skip3, teste_9 = conformerConvModule(encoder_2, n_filtro*4,11)
    
    skip4, encoder_4 =  merge_PPM(encoder_3,teste_7,teste_8,teste_9,n_filtro*8)
...
def conformerConvModule(model_input,filters=48, kernel_size=3):
    dim = filters
